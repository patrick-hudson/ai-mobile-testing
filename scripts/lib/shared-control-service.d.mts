...
import type { PromotionAuthorityContext } from './promotion-claim-store.mjs';
...
export interface SharedControlService {
  ...
  withPublicationFence<T>(principal: ControlPrincipal, runId: string,
    callback: (publication: any, authorityContext: PromotionAuthorityContext) => T | Promise<T>): Promise<T>;
  withReleaseAssertionFence<T>(principal: ControlPrincipal, runId: string,
    callback: (publication: any, authorityContext: PromotionAuthorityContext) => T | Promise<T>): Promise<T>;
